...
st.write(
    "まずは左の設定から専門家モードを選択し、下の入力欄に業務の状況や課題を入力してください。おすすめの使い方は、最初に業務分析スペシャリストに対して現状の業務プロセスを詳しく説明し、非効率やリスクを洗い出してもらうことです。その後、生成AI・SaaS活用コンサルタントに切り替えて、具体的な改善策を提案してもらう流れが効果的です。"
)

st.write("業務分析スペシャリスト入力例：営業報告書の作成に毎週3時間かかっています。Excelで管理していますが...")

st.write("生成AI・SaaS活用コンサルタント入力例：業務分析スペシャリストの回答をコピーしたもの")
...
user_input = st.chat_input(
    "例：営業報告書の作成に毎週3時間かかっています。Excelで管理していますが..."
)
